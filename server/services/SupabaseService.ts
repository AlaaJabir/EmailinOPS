...
export class SupabaseService {
  ...
  async verifyToken(token: string): Promise<AuthenticatedUser | null> {
    ...
    if (this.isConfigured && this.client) {
      try {
        ...
        try {
          const { data: profile } = await this.client.from('profiles').select('*').eq('id', su.id).maybeSingle();
          if (profile) {
            profileRole = profile.role || profileRole;
            profileName = profile.full_name || profileName;
            profilePlan = profile.plan || profilePlan;
          } else {
            await this.client.from('profiles').insert({ id: su.id, email: su.email || '', full_name: profileName, role: profileRole, plan: profilePlan }).maybeSingle();
          }
        } catch { /* non-blocking profile fallback */ }
        return { id: su.id, email: su.email || '', name: profileName, role: profileRole, plan: profilePlan };
      } catch (err) {
    ...
    if (cleanToken.startsWith('test-') || cleanToken === 'mock-jwt-token') {
      ...
      return { id: existingUser?.id || 'usr_admin_01', email: existingUser?.email || 'admin@emailops.io', name: existingUser?.name || 'Alex Vance (Lead Email Architect)', role: (existingUser?.role as any) || 'ADMIN', plan: 'PRO' };
    }
    return null;
  }
  ...
  async isEventProcessed(eventId: string): Promise<boolean> {
    ...
    if (db.hasProcessedEvent(eventId)) return true;
    if (this.isConfigured && this.client) {
      try {
        const { data, error } = await this.client.from('processed_webhook_events').select('event_id').eq('event_id', eventId).maybeSingle();
        if (!error && data) { db.recordProcessedEvent(eventId); return true; }
      } catch (err) { console.warn('[SupabaseService] isEventProcessed query failed:', err); }
    }
    return false;
  }
  ...
  async recordProcessedEvent(eventId: string, provider = 'SES_SNS'): Promise<void> {
    ...
    if (this.isConfigured && this.client) {
      try { await this.client.from('processed_webhook_events').insert({ event_id: eventId, provider }); } catch { /* already processed is harmless */ }
    }
  }
  ...
  async saveMessage(message: Message, userId: string): Promise<void> {
    ...
    if (this.isConfigured && this.client) {
      try {
        ...
        if (error) console.error('[SupabaseService] Error saving message to Supabase:', error.message);
      } catch (err) { console.error('[SupabaseService] Exception saving message to Supabase:', err); }
    }
  }
  ...
  async updateMessageStatus(params: { messageId: string; status: Message['status']; sesMessageId?: string; deliveredAt?: string; bouncedAt?: string; bounceType?: 'Hard' | 'Soft' | 'Transient'; bounceReason?: string; smtpResponse?: string; }): Promise<void> {
    ...
    if (this.isConfigured && this.client) {
      try {
        ...
        if (error) console.error('[SupabaseService] Error updating message status:', error.message);
      } catch (err) { console.error('[SupabaseService] Exception updating message status in Supabase:', err); }
    }
  }
    ...
    if (this.isConfigured && this.client) {
      try {
        const { error } = await this.client.from('message_events').insert({
          message_id: event.messageId, user_id: userId || null, event_type: event.eventType, event_data: event.eventData || null,
          ip_address: event.ipAddress || null, user_agent: event.userAgent || null, geo: event.geo || null, timestamp: event.timestamp || new Date().toISOString(),
        });
        if (error) console.error('[SupabaseService] Error saving message event:', error.message);
      } catch (err) { console.error('[SupabaseService] Exception saving event in Supabase:', err); }
    }
  }
  ...
  async saveTechnicalLog(log: TechnicalLog, userId?: string): Promise<void> {
    ...
    if (this.isConfigured && this.client) {
      try {
        ...
        if (error) console.error('[SupabaseService] Error saving technical log:', error.message);
      } catch (err) { console.error('[SupabaseService] Exception saving technical log:', err); }
    }
  }
  ...
  async getMessages(userId: string, limit = 100): Promise<Message[]> {
    if (this.isConfigured && this.client) {
      try {
        ...
        if (!error && data) {
          const messageIds = data.map((row: any) => row.message_id).filter(Boolean);
          let events: any[] = [];
          if (messageIds.length) {
            const eventResult = await this.client.from('message_events').select('*').in('message_id', messageIds).order('timestamp', { ascending: true });
            if (!eventResult.error && eventResult.data) events = eventResult.data;
            else if (eventResult.error) console.error('[SupabaseService] Error loading message events:', eventResult.error.message);
          }
          const eventsByMessage = new Map<string, MessageEvent[]>();
          for (const row of events) {
            const mapped: MessageEvent = {
              id: row.id || `evt_${row.message_id}_${row.timestamp}`,
            ...
            const list = eventsByMessage.get(mapped.messageId) || [];
            list.push(mapped);
            eventsByMessage.set(mapped.messageId, list);
          }
          return data.map((row: any) => ({
            id: row.internal_id || row.id, messageId: row.message_id, sesMessageId: row.ses_message_id, campaignId: row.campaign_id,
            senderId: row.sender_id, fromName: row.from_name, fromEmail: row.from_email, toEmail: row.to_email, replyTo: row.reply_to,
            cc: row.cc || [], bcc: row.bcc || [], subject: row.subject, htmlBody: row.html_body, plainText: row.plain_text,
            customHeaders: row.custom_headers, status: row.status, provider: row.provider, providerMessageId: row.provider_message_id,
            smtpResponse: row.smtp_response, bounceType: row.bounce_type, bounceReason: row.bounce_reason, queuedAt: row.queued_at,
            sentAt: row.sent_at, deliveredAt: row.delivered_at, bouncedAt: row.bounced_at, createdAt: row.created_at,
            events: eventsByMessage.get(row.message_id) || [],
          }));
        }
        if (error) console.error('[SupabaseService] Error querying messages from Supabase:', error.message);
      } catch (err) { console.error('[SupabaseService] Error querying messages from Supabase:', err); }
    }
  ...
  async getSenders(userId: string): Promise<Sender[]> {
    if (this.isConfigured && this.client) {
      try {
        const { data, error } = await this.client.from('senders').select('*').eq('user_id', userId).order('created_at', { ascending: false });
        if (!error && data) return data.map((row: any) => ({ id: row.id, name: row.name, fromEmail: row.from_email, replyTo: row.reply_to, domainId: row.domain_id, status: row.status, verification: row.verification, dailyLimit: row.daily_limit, hourlyLimit: row.hourly_limit, sentCount: row.sent_count, deliveredCount: row.delivered_count, bouncedCount: row.bounced_count, complaintCount: row.complaint_count, createdAt: row.created_at, updatedAt: row.updated_at }));
      } catch (err) { console.error('[SupabaseService] Error querying senders:', err); }
    }
    return db.senders;
  }
  ...
  async getCampaigns(userId: string): Promise<Campaign[]> {
    if (this.isConfigured && this.client) {
      try {
        ...
        if (!error && data) return data.map((row: any) => ({ id: row.id, name: row.name, senderId: row.sender_id, listId: row.list_id, subject: row.subject, htmlBody: row.html_body, plainText: row.plain_text, status: row.status, scheduledAt: row.scheduled_at, startedAt: row.started_at, completedAt: row.completed_at, totalRecipients: row.total_recipients, sentCount: row.sent_count, deliveredCount: row.delivered_count, bouncedCount: row.bounced_count, complaintCount: row.complaint_count, openCount: row.open_count, clickCount: row.click_count, createdAt: row.created_at, updatedAt: row.updated_at }));
      } catch (err) { console.error('[SupabaseService] Error querying campaigns:', err); }
    }
    return db.campaigns;
  }
  ...
  async getContacts(userId: string): Promise<Contact[]> {
    if (this.isConfigured && this.client) {
      try {
        ...
        if (!error && data) return data.map((row: any) => ({ id: row.id, email: row.email, firstName: row.first_name, lastName: row.last_name, company: row.company, tags: row.tags || [], status: row.status, bounceReason: row.bounce_reason, createdAt: row.created_at, updatedAt: row.updated_at }));
      } catch (err) { console.error('[SupabaseService] Error querying contacts:', err); }
    }
    return db.contacts;
  }
  ...
  async getSuppressions(userId?: string): Promise<SuppressionItem[]> {
    if (this.isConfigured && this.client && userId) {
      try {
        ...
        if (!error && data) return data.map((row: any) => ({ id: row.id, email: row.email, type: row.type, reason: row.reason, source: row.source, createdAt: row.created_at }));
      } catch (err) { console.error('[SupabaseService] Error querying suppressions:', err); }
    }

...
/** Record at most one OPENED and one CLICKED event per message. */
function recordUniqueTrackingEvent(params: {
  ...
  const alreadyRecorded = (message.events || []).some((event) => event.eventType === params.eventType) ||
    db.messageEvents.some((event) => event.messageId === message.messageId && event.eventType === params.eventType);

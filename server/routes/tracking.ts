...
async function recordTrackingEvent(params: {
  ...
  if (message) {
    const alreadyRecorded =
      (message.events || []).some((event) => event.eventType === params.eventType) ||
      db.messageEvents.some(
        (event) => event.messageId === message.messageId && event.eventType === params.eventType
      );
  ...
  // Also persist event via convexService
  const eventRecord = {
  ...
  await convexService.saveMessageEvent(eventRecord).catch(() => {});
}
...
trackingRouter.get('/click/:messageId', async (req: Request, res: Response) => {
  ...
  try {
    await recordTrackingEvent({
      messageId,
      eventType: 'CLICKED',
      eventData: { destinationUrl, ip: req.ip, userAgent: req.get('user-agent') },
      ipAddress: req.ip,
...
trackingRouter.get('/open/:messageId', async (req: Request, res: Response) => {
  ...
  try {
    await recordTrackingEvent({
      messageId,
      eventType: 'OPENED',
      eventData: { ip: req.ip, userAgent: req.get('user-agent') },
      ipAddress: req.ip,
      userAgent: req.get('user-agent'),
    });
  } catch (err) {

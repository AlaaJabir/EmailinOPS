...
export function App() {
  ...
  const jsonAction = async (url:string, body:any, success:string) => { try { const r=await authFetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); const text=await r.text(); let d:any={}; try{d=text?JSON.parse(text):{};}catch{d={};} if(!r.ok){addToast('error','Request Failed',d.error||'Request failed');return false;} addToast('success',success); await refreshAll(); return true; } catch(e:any){addToast('error','Connection Error',e.message);return false;} };

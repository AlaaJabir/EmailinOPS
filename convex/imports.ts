...
export const start = mutation({
  ...
  handler: async (ctx, args) => {
    const listId = await ctx.db.insert("contact_lists", {
      userId: args.userId,
      name: args.listName,
      description: args.listDescription,
      contactCount: 0,
      createdAt: args.now,
    });
  ...
  handler: async (ctx, args) => {
    ...
    for (const row of args.rows) {
      ...
      const membership = await ctx.db
        ...
        .withIndex("by_listId_contactId", (q) => q.eq("listId", job.listId).eq("contactId", contactId))
        .first();
      if (!membership) {

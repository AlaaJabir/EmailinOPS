...
-- ============================================================
-- 2) Campaign recipients: one durable row per campaign recipient
-- ============================================================
...
-- ============================================================
-- 4) Persistent application settings
-- ============================================================
...
-- ============================================================
-- 5) Campaign fields used by the professional composer
-- ============================================================
...
-- ============================================================
-- 6) Message fields useful for durable tracking / campaign linkage
-- ============================================================
...
-- ============================================================
-- 7) Normalize verification defaults for newly-created domains.
-- Existing rows are intentionally NOT modified here.
-- ============================================================
alter table public.domains
  alter column spf_status set default 'PENDING',
  alter column dkim_status set default 'PENDING',
  alter column dmarc_status set default 'PENDING',
  alter column ses_status set default 'PENDING';

-- ============================================================
-- 8) Useful uniqueness and lookup indexes
-- ============================================================
create unique index if not exists templates_user_name_uidx
  on public.templates (user_id, name);

create unique index if not exists unsubscribe_tokens_token_uidx
...
create unique index if not exists senders_user_email_uidx
  on public.senders (user_id, lower(from_email));
...
create index if not exists messages_to_email_idx
  on public.messages (lower(to_email));

create index if not exists message_events_message_type_idx
  on public.message_events (message_id, event_type, timestamp desc);

create index if not exists message_events_user_type_idx
  on public.message_events (user_id, event_type, timestamp desc);

create index if not exists suppressions_user_email_idx
  on public.suppressions (user_id, lower(email));

create index if not exists technical_logs_user_timestamp_idx
  on public.technical_logs (user_id, timestamp desc);

create index if not exists settings_user_category_idx
  on public.settings (user_id, category);

-- ============================================================
-- 9) Basic integrity constraints (NOT VALID avoids failing on
-- existing dirty rows; validate later after a data audit).
-- ============================================================
DO $$
BEGIN
  IF NOT EXISTS (
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'campaign_recipients_contact_id_fkey'
  ) THEN
    ALTER TABLE public.campaign_recipients
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'campaign_recipients_message_id_fkey'
  ) THEN
    ALTER TABLE public.campaign_recipients
...
-- ============================================================
-- 10) RLS: backend uses the Supabase service role, which bypasses
-- RLS. No public/anon access policies are created here.
-- ============================================================
...
-- ============================================================
-- 11) Existing webhook dedup table is the canonical processed-event
-- store. We intentionally do NOT create a second processed_events table.
-- ============================================================

...
async function dashboardStats(userId: string, period: Period = '30d') {
  ...
  let [messagesRaw, sendersRaw, campaignsRaw] = await Promise.all([
    convexService.getMessages(userId),
    convexService.getSenders(userId),
    convexService.getCampaigns(userId),
  ]);

  const allMessages = (messagesRaw && messagesRaw.length ? messagesRaw : db.messages).map((m: any) => ({
    id: m.id,
    internal_id: m.internalId || m.id,
  ...
  const allEvents = db.messageEvents.map((e: any) => ({
  ...
  const senderRows = (sendersRaw && sendersRaw.length ? sendersRaw : db.senders).map((s: any) => ({
    id: s.id,
    name: s.name,
    from_email: s.fromEmail || s.from_email,
  }));

  const campaignRows = (campaignsRaw && campaignsRaw.length ? campaignsRaw : db.campaigns).map((c: any) => ({
    id: c.id,
    name: c.name,
    sent_count: c.sentCount || c.sent_count || 0,
    delivered_count: c.deliveredCount || c.delivered_count || 0,
    open_count: c.openCount || c.open_count || 0,
    click_count: c.clickCount || c.click_count || 0,
    created_at: c.createdAt || c.created_at || new Date().toISOString(),
    status: c.status,
    total_recipients: c.totalRecipients || c.total_recipients || 0,
  }));
  ...
  const topSenders = senderRows.map((s: any) => {
    const ms = messages.filter((m: any) => m.sender_id === s.id);
    const sv = ms.filter((m: any) => ['QUEUED', 'SENDING', 'SENT', 'DELIVERED'].includes(m.status)).length;
    const del = ms.filter((m: any) => (m.status === 'DELIVERED' || m.status === 'SENT') && m.status !== 'BOUNCED').length;
    return {
      id: s.id,
      name: s.name,
      email: s.from_email,
      volume: sv,
      deliveryRate: sv ? Number(((del / sv) * 100).toFixed(2)) : 0,
      bounceRate: sv ? Number(((ms.filter((m: any) => m.status === 'BOUNCED').length / sv) * 100).toFixed(2)) : 0,
    };
  }).sort((a: any, b: any) => b.volume - a.volume).slice(0, 10);

  let topCampaigns = campaignRows.map((c: any) => ({
    id: c.id,
    name: c.name,
    sent: c.sent_count || 0,
    delivered: c.delivered_count || 0,
    openRate: c.sent_count ? Number((((c.open_count || 0) / c.sent_count) * 100).toFixed(2)) : 0,
    clickRate: c.sent_count ? Number((((c.click_count || 0) / c.sent_count) * 100).toFixed(2)) : 0,
    status: c.status,
    totalRecipients: c.total_recipients || 0,
    createdAt: c.created_at,
  })).sort((a: any, b: any) => b.sent - a.sent).slice(0, 10);
...
async function startServer() {
  ...
  app.get('/api/health', async (_req, res) => {
    const [kumoHealth, sesHealth, database] = await Promise.all([
      ...
      convexService.health(),
    ]);
    const status = kumoHealth.status === 'healthy' ? 'healthy' : 'degraded';
    res.status(status === 'healthy' ? 200 : 503).json({
      status,
      timestamp: new Date().toISOString(),
      services: { api: 'healthy', database, kumomta: kumoHealth, amazon_ses: sesHealth },
    });
  });
  app.get('/api/health/kumomta', async (_req, res) => {
    const k = await kumoMtaService.checkHealth();
    res.status(k.status === 'healthy' ? 200 : 503).json(k);
  });
  app.get('/api/dashboard/stats', optionalAuth, async (req, res) => {
    try {
      const raw = String(req.query.period || '30d');
      const period: Period = raw === 'today' || raw === '7d' || raw === '30d' ? raw : '30d';
      const userId = req.user?.id || (await convexService.getDefaultUserId());
      return res.json(await dashboardStats(userId, period));
    } catch (err: any) {

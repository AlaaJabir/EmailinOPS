...
describe('KumoMTA Real Integration - Step 1 Suite', () => {
  describe('1. Configuration Validation', () => {
    ...
    it('should fail validation when port is invalid', () => {
      ...
      const result = service.validateConfig();
      assert.strictEqual(result.valid, false);
      assert.ok(result.errors.some((e) => e.includes('port must be between')));
    ...
    it('should sanitize config and NEVER expose password in getConfigSanitized', () => {
      const service = new KumoMtaService({ host: 'mta.acme.internal', port: 587, username: 'kumo_admin_user', password: 'SUPER_SECRET_PASSWORD_123' });
  ...
  describe('2. Health Check', () => {
    ...
    it('should report healthy when transporter verify succeeds', async () => {
      ...
      const mockTransporter: any = { verify: async () => true, sendMail: async () => ({ response: '250 OK' }) };
    ...
    it('should report offline with error message when transporter verify fails', async () => {
      const service = new KumoMtaService({ host: '127.0.0.1', port: 25 });
      const mockTransporter: any = { verify: async () => { throw new Error('Connection refused: 127.0.0.1:25'); } };
      service.setTransporter(mockTransporter);
      const health = await service.checkHealth();
      assert.strictEqual(health.status, 'offline');
      assert.strictEqual(health.configured, true);
  ...
  describe('4. Real Email Submission Path & Status', () => {
    it('should submit email to transporter and mark initial status as QUEUED (not fake DELIVERED)', async () => {
      ...
      const mockTransporter: any = {
        sendMail: async (options: any) => {
          ...
          return { messageId: options.messageId, response: '250 2.0.0 OK: queued in KumoMTA spool id spool_9921', accepted: [options.to], rejected: [] };
        },
      };
      service.setTransporter(mockTransporter);
      const payload: SendEmailPayload = { fromName: 'Acme Alerts', fromEmail: 'alerts@transact.acme-corp.io', to: 'developer@example.com', subject: 'System Spool Test', htmlBody: '<p>Test message</p>', plainText: 'Test message' };
    ...
    it('should handle SMTP 550 rejection and update status to FAILED', async () => {
      const service = new KumoMtaService({ host: '127.0.0.1', port: 25 });
      const mockTransporter: any = { sendMail: async () => { const err: any = new Error('550 5.1.1 Recipient address rejected: User unknown'); err.responseCode = 550; throw err; } };
      service.setTransporter(mockTransporter);
      await assert.rejects(async () => service.submitEmail({ fromEmail: 'security@transact.acme-corp.io', to: 'nonexistent@invalid-domain.com', subject: 'Bounce Test' }), /550 5.1.1 Recipient address rejected/);
    ...
    it('should submit successfully when db.settings is undefined', async () => {
      ...
      try {
        ...
        const service = new KumoMtaService({ host: '127.0.0.1', port: 25 });
        const mockTransporter: any = { sendMail: async (options: any) => ({ messageId: options.messageId, response: '250 2.0.0 OK: queued in KumoMTA spool', accepted: [options.to], rejected: [] }) };
        service.setTransporter(mockTransporter);
        const result = await service.submitEmail({ fromEmail: 'alerts@transact.acme-corp.io', to: 'optional-settings@example.com', subject: 'Optional Settings Regression' });
        assert.strictEqual(result.success, true);
        assert.strictEqual(result.status, 'QUEUED');
      } finally {
  ...
  describe('5. Security: Zero Password Leaks in Logs', () => {
    it('should never write secrets or passwords to database logs', async () => {
      ...
      const service = new KumoMtaService({ host: '127.0.0.1', port: 25, username: 'admin', password: secretPassword });
      const mockTransporter: any = { sendMail: async (options: any) => ({ messageId: options.messageId, response: '250 OK: queued in spool', accepted: [options.to], rejected: [] }) };
      service.setTransporter(mockTransporter);
      await service.submitEmail({ fromEmail: 'security@transact.acme-corp.io', to: 'audit@example.com', subject: 'Security Log Audit' });
      for (const log of db.logs) assert.strictEqual(JSON.stringify(log).includes(secretPassword), false);
    });
  });
  ...
  describe('6. Production Send Path Has No Mock Logic', () => {
    it('should NOT determine delivery status based on "bounce" or "fail" in recipient email when using real transporter', async () => {
      ...
      const mockTransporter: any = { sendMail: async (options: any) => { sendMailInvoked = true; return { messageId: options.messageId, response: '250 OK: accepted into KumoMTA spool', accepted: [options.to], rejected: [] }; } };
      service.setTransporter(mockTransporter);
      const result = await service.submitEmail({ fromEmail: 'security@transact.acme-corp.io', to: 'user-bounce-testing@example.com', subject: 'Real Submission Test' });
